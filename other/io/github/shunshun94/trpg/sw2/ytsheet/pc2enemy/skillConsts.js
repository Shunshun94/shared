...
io.github.shunshun94.trpg.sw2.ytsheet.PC2ENEMY.CONSTS.RACE_ABILITY.LIST = {
  ...
  "通じ合う意識": {
    replaceFunction: (json) => {
      const level = Number(json.level);
      if(level < 6) {
        return `≫通じ合う意識&lt;br&gt;1分（6ラウンド）の間、自らを中心に「半径10m（2～3エリア）」の範囲内に存在する「分類：動物」「分類：幻獣」のキャラクターを選び、全ての部位の命中力・回避力判定に+1のボーナス修正を与えます。この効果は6時間ごとに1回だけ使えます。`;
      } else if(level < 11) {
        return `≫通じ合う意識&lt;br&gt;1分（6ラウンド）の間、自らを中心に「半径30m（全エリア）」の範囲内に存在する「分類：動物」「分類：幻獣」のキャラクターを選び、全ての部位の命中力・回避力判定に+1のボーナス修正を与えます。この効果は6時間ごとに1回だけ使えます。`;
      } else {
        return `≫通じ合う意識&lt;br&gt;1分（6ラウンド）の間、自らを中心に「半径30m（全エリア）」の範囲内に存在する「分類：動物」「分類：幻獣」のキャラクターを選び、全ての部位の命中力・回避力判定に+2のボーナス修正を与えます。この効果は6時間ごとに1回だけ使えます。`;
      }
    }
  },
  "無生物と通じ合う意識": {
    replaceFunction: (json) => {
      const level = Number(json.level);
      if(level < 6) {
        return `≫無生物と通じ合う意識&lt;br&gt;1分（6ラウンド）の間、自らを中心に「半径10m（2～3エリア）」の範囲内に存在する「分類：魔動機」「分類：魔法生物」のキャラクターを選び、全ての部位の命中力・回避力判定に+1のボーナス修正を与えます。この効果は6時間ごとに1回だけ使えます。`;
      } else if(level < 11) {
        return `≫無生物と通じ合う意識&lt;br&gt;1分（6ラウンド）の間、自らを中心に「半径30m（全エリア）」の範囲内に存在する「分類：魔動機」「分類：魔法生物」のキャラクターを選び、全ての部位の命中力・回避力判定に+1のボーナス修正を与えます。この効果は6時間ごとに1回だけ使えます。`;
      } else {
        return `≫無生物と通じ合う意識&lt;br&gt;1分（6ラウンド）の間、自らを中心に「半径30m（全エリア）」の範囲内に存在する「分類：魔動機」「分類：魔法生物」のキャラクターを選び、全ての部位の命中力・回避力判定に+2のボーナス修正を与えます。この効果は6時間ごとに1回だけ使えます。`;
      }
    }
  },
